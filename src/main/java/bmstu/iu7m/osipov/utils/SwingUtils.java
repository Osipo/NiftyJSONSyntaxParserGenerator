 ...
 *
 * @author Darryl Burke
 * Approach of extracting Components by type from the Composite Swing Components
 * is described by this author at: https://tips4java.wordpress.com/2008/11/13/swing-utils/
 * The source code: http://www.camick.com/java/source/SwingUtils.java
 */
public final class SwingUtils {
    ...
    public static <T extends JComponent> List<T> getDescendantsOfType2(Class<T> clazz, Container parent){
        LinkedStack<Component> comps = new LinkedStack<>();
        ...
        while(!comps.isEmpty()){
            ...
            if(clazz.isAssignableFrom(current.getClass()) || current.getClass().getSimpleName().equals(clazz.getSimpleName())){
                result.add(clazz.cast(current));
            }
            if(current instanceof Container && ((Container) current).getComponents().length > 0){
                children = ((Container) current).getComponents();
                for(int i = 0; i < children.length; i++){
                    comps.push(children[i]);
                }
            }
        }
        return result;

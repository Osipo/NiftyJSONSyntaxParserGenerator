...
public class BaseInterpreter {
    ...
    private void execFunction(FunctionInterpreter f, PositionalTree<AstSymbol> ast, LinkedStack<String> exp){
        AtomicReference<Env> env = new AtomicReference<>();
        env.set(f.getContext());
    ...
    private void applyOperation(PositionalTree<AstSymbol> ast, AtomicReference<Env> context, Node<AstSymbol> cur,
        ...
        switch (opType){
            ...
            case "lambda": {
                if(ast.parent(cur).getValue().getType().equals("list")){ //lambda in list/items.
